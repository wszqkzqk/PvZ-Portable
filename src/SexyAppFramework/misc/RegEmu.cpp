...
static void SaveToFile()
{
	if (currFile.empty())
	{
		Sexy::LogInfoLn("RegEmu: Filename not specified, can't save");
		return;
	}
	...
	if (!f)
	{
		Sexy::LogInfoLn("RegEmu: Couldn't open '{}' for writing", currFile);
		return;
	}
...
void regemu::SetRegFile(const std::string& fileName)
{
	...
	if (!f)
	{
		Sexy::LogInfoLn("RegEmu: Can't read '{}': File does not exist", currFile);
		return;
	}
	...
	{
		Sexy::LogInfoLn("RegEmu: Can't read '{}': Invalid header", currFile);
		return;
	}
	...
	Sexy::LogInfoLn("RegEmu: Loaded from '{}': {} total key(s)", currFile, static_cast<size_t>(registry.size()));
}
...
bool regemu::RegistryRead(const std::string& keyName, const std::string& valueName, uint32_t* type, uint8_t* value, uint32_t* length)
{
	...
	{
		Sexy::LogInfoLn("RegEmu: Key '{}' does not exist", keyName);
		return false;
	}
	if (!registry[keyName].count(valueName))
	{
		Sexy::LogInfoLn("RegEmu: Value '{}' does not exist", valueName);
		return false;
	}
...
bool regemu::RegistryEraseKey(const std::string& keyName)
{
	...
	Sexy::LogInfoLn("RegEmu: Erased key '{}'", keyName);
...
bool regemu::RegistryEraseValue(const std::string& keyName, const std::string& valueName)
{
	...
	Sexy::LogInfoLn("RegEmu: Erased value '{}' from key '{}'", valueName, keyName);
